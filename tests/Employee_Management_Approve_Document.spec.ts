...
test.describe("'Employee Management > Approve Document module', @reg", () => {
  ...
  test("compares approved document types in UI with dropdown options @reg", async ({
    ...
    // Exit if no records
    if (
      (await EmployeeDirectory.Approve_Label_Exiting_Document_type.count()) ===
      0
    ) {
  ...
  test("compares unique employee types with dropdown options", async ({
    ...
    const dropdownTypes = await EmployeeDirectory.getUniqueTextSetFromLocator(
      page.locator('[id^="react-select-3-option"]'),
      1
    );
  ...
  test("filters employee records based on dropdown selection", async ({
    ...
    const selectedOption = await page
      .locator("(//div[@class = ' css-1dimb5e-singleValue'])[2]")
      .textContent();
    console.log("Selected Dropdown Option:", selectedOption);

    await page.waitForTimeout(1500);
    const filteredTypes = await EmployeeDirectory.getUniqueTextSetFromLocator(
      EmployeeDirectory.Approve_Label_Exiting_Employee
    );
  ...
  test("should show validation message when action is not selected, @reg", async ({
    ...
    const message = await EmployeeDirectory.getValidationMessage(
      EmployeeDirectory.Select_action_dropdown
    );
  ...
  test("should change document status to approved and show success toast @smoke, @reg", async ({
    ...
    await EmployeeDirectory.Action_button.click();
    await EmployeeDirectory.waitforLoaderToDisappear();
    await EmployeeDirectory.Select_action_dropdown.selectOption({
      value: "approved",
    });
    await EmployeeDirectory.PopUP_Submit_button.click();
    await EmployeeDirectory.waitforLoaderToDisappear();
  ...
  test("should validate rejection reason and change status to rejected @smoke, @reg", async ({
    ...
    await EmployeeDirectory.PopUP_Submit_button.click();
    let message1 = await EmployeeDirectory.getValidationMessage(
      EmployeeDirectory.Reason_Section
    );
    expect(
      message1 === FILL_OUT_FIELD || message1 === FILL_IN_FIELD
    ).toBeTruthy();
