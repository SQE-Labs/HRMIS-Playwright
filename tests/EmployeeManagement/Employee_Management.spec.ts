...
test.describe("'Employee Management module'", () => {
    ...
    test("Verify Search Functionality and Validate Employee Card Details and Count", async ({ page }) => {
        await EmployeeDirectory.searchByEmployeeDirectorySearchBar("Elon Mask")
        let { TotalCards, TotalEmployeecount } = await EmployeeDirectory.totalCardsCount()
        var TotalCardsText = await EmployeeDirectory.Employee_Directory_cards_title.textContent() || ""
        expect(TotalCardsText.trim().replace(/\s+/g, ' ')).toContain("Elon Mask")
        expect(TotalCards).toEqual(TotalEmployeecount)
    })
    ...
    test("verify Status Filter Results With Or Without Employees @smoke @reg", async ({ page }) => {
        await EmployeeDirectory.optionSelection(EmployeeDirectory.SelectDepartment, 'Technical')
        await EmployeeDirectory.waitforLoaderToDisappear()
        await EmployeeDirectory.optionSelection(EmployeeDirectory.SelectStatus, 'BLOCKED (Temporarily Disable)')
        await EmployeeDirectory.waitforLoaderToDisappear()
        let { TotalCards, TotalEmployeecount } = await EmployeeDirectory.totalCardsCount()
        if (TotalCards === 0) {
            var Norecord = await EmployeeDirectory.noRecord(1)
    ...
    test("verify Validation Message When Date Of Joining Is Cleared In Work @reg", async ({ page }) => {
        await EmployeeDirectory.clickOnEmployeeCard()
        await EmployeeDirectory.waitforLoaderToDisappear()
        await EmployeeDirectory.clickOnWorkAccordion()
    ...
    // Out of scope test case
    // test("verify Validation Message When EmployeeType no Option is selected In Work", async ({ page }) => {
    //     await EmployeeDirectory.clickOnEmployeeCard()
    //     await EmployeeDirectory.waitforLoaderToDisappear()
    //     await EmployeeDirectory.clickOnWorkAccordion()
    //     await EmployeeDirectory.clickOnWorkEditButton()
    //     await EmployeeDirectory.EmployeeType.selectOption({ label: 'Select Employee Type' })
    //     await EmployeeDirectory.clickOnUpdateButton()
    ...
    test("verify Work Remains Unchanged After Closing Edit Form", async ({ page }) => {
        ...
        expect(existingInfo.ExistingDate).toEqual(updatedInfo.CurrentDate)
        expect(existingInfo.employeeType).toEqual(updatedInfo.employeeType)
        expect(existingInfo.employeeSubType).toEqual(updatedInfo.employeeSubType)
    })

    test("verify Profile Update With Random Work Date Info Fields @reg @bug", async ({ page }) => {
        await EmployeeDirectory.clickOnEmployeeCard()
        await EmployeeDirectory.waitforLoaderToDisappear()
        await EmployeeDirectory.clickOnWorkAccordion()
        await page.pause();
    ...
    test("verify Profile Update With Random Peronal Details clicking on closw button @reg ", async ({ page }) => {
        ...
        expect(aadharNumber).toEqual(currentaadharNumber)
        expect(PanCardNumber).toEqual(currentPanCardNumber)
        expect(permanentAddress).toEqual(CurrentpermanentAddress)
        expect(DateofBirth).toEqual(currentDateofBirth)
        expect(PassportNumber).toEqual(currentPassportNumber)
        expect(PresentAddress).toEqual(currentPresentAddress)
        expect(BloodGroup).toEqual(currentBloodGroup)
        expect(MaritalStatus).toEqual(currentMaritalStatus)
        expect(AlternateNumber).toEqual(currentAlternateNumber)
        expect(Gender).toEqual(currentGender)
    })
    ...
    test("verify Profile Update With Random Perosnal details Info Fields @reg", async ({ page }) => {
        ...
        expect(permanentAddress).not.toEqual(CurrentpermanentAddress)
        expect(DateofBirth).not.toEqual(currentDateofBirth)
        expect(PassportNumber).not.toEqual(currentPassportNumber)
        expect(PresentAddress).not.toEqual(currentPresentAddress)
        expect(BloodGroup).not.toEqual(currentBloodGroup)
        expect(MaritalStatus).not.toEqual(currentMaritalStatus)
        expect(AlternateNumber).not.toEqual(currentAlternateNumber)
        expect(Gender).not.toEqual(currentGender)
    })
