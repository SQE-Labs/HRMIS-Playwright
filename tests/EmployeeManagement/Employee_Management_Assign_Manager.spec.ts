...
test.describe("'Employee Management > Assign Manager module'", () => {
    ...
    test("should open Assign Manager popup after selecting employee and clicking action button @reg", async ({ page }) => {
        ...
        await EmployeeDirectory.selectEmployeeOption.click()
        await EmployeeDirectory.waitforLoaderToDisappear()
        await EmployeeDirectory.clickOnReassignActionButton()
    ...
    test("should close Assign Manager popup when cross button is clicked @reg", async ({ page }) => {
        ...
        await EmployeeDirectory.waitforLoaderToDisappear()
        await EmployeeDirectory.clickOnReassignActionButton()
        await expect(EmployeeDirectory.ActiveModal).toBeVisible()
        await EmployeeDirectory.clickOnCrossButton(EmployeeDirectory.ActiveModalClose)
        await expect(EmployeeDirectory.ActiveModal).toBeHidden()
    ...
    test("should close Assign Manager popup when cancel button is clicked @reg", async ({ page }) => {
        ...
        await EmployeeDirectory.clickOnReassignActionButton()
        await expect(EmployeeDirectory.ActiveModal).toBeVisible()
        await EmployeeDirectory.clickOnCancelButton(EmployeeDirectory.ActiveModalCancel)
        await expect(EmployeeDirectory.ActiveModal).toBeHidden()
    ...
    test.fixme("conditionally assign and delete leave manager, then verify removal @smoke @bug ", async ({ page }) => {
        ...
        let Coloumncount = await EmployeeDirectory.getColumnCount()
        if (Coloumncount === 0) {
            let No_record = await EmployeeDirectory.No_Record.textContent()
            expect(No_record).toEqual("Leave manager not assigned")
            await EmployeeDirectory.AssignButton.click()
            await EmployeeDirectory.waitforLoaderToDisappear()
            await EmployeeDirectory.LeaveManagerPopupDropDown.click()
