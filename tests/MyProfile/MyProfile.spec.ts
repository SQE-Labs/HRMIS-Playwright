...
import {
  MY_PROFILE_USER_EMAIL,
  MY_PROFILE_USER_PASSWORD,
  MY_PROFILE_TEMP_PASSWORD,
  WORK_EXPERIENCE_TEST_DATA,
  EDUCATION_TEST_DATA,
  DEPENDENT_TEST_DATA,
...
test('HRMIS_EM_4 Verify General Information accordions are visible', async ({ page }) => {
  ...
  await myProfilePage.expandAccordion(myProfilePage.educationAccordion);
  await myProfilePage.expandAccordion(myProfilePage.dependentsAccordion);
});
...
test('HRMIS_EM_6 Verify Add and Delete Education on My Profile', async ({ page }) => {
  const loginPage = new LoginPage(page);
  await loginPage.validLogin(MY_PROFILE_USER_EMAIL, MY_PROFILE_USER_PASSWORD);

  const myProfilePage = new MyProfilePage(page);
  await myProfilePage.openMyProfile();
  await myProfilePage.openEducationSection();

  await myProfilePage.clickAddEducation();
  await myProfilePage.fillEducationForm(EDUCATION_TEST_DATA);
  await myProfilePage.submitEducationForm();
  await myProfilePage.verifyEducationRowExists(EDUCATION_TEST_DATA.degree);
  await myProfilePage.deleteEducationRow(EDUCATION_TEST_DATA.degree);
  await myProfilePage.verifyEducationRowNotExists(EDUCATION_TEST_DATA.degree);
});

test('HRMIS_EM_7 Verify Add and Delete Dependent on My Profile', async ({ page }) => {
  const loginPage = new LoginPage(page);
  await loginPage.validLogin(MY_PROFILE_USER_EMAIL, MY_PROFILE_USER_PASSWORD);

  const myProfilePage = new MyProfilePage(page);
  await myProfilePage.openMyProfile();
  await myProfilePage.openDependentsSection();

  await myProfilePage.clickAddDependent();
  await myProfilePage.fillDependentForm(DEPENDENT_TEST_DATA);
  await myProfilePage.submitDependentForm();
  await myProfilePage.verifyDependentRowExists(DEPENDENT_TEST_DATA.name);
  await myProfilePage.deleteDependentRow(DEPENDENT_TEST_DATA.name);
  await myProfilePage.verifyDependentRowNotExists(DEPENDENT_TEST_DATA.name);
});

test('HRMIS_EM_8 Verify Appraisal History View downloads PDF', async ({ page }) => {
  const loginPage = new LoginPage(page);
  await loginPage.validLogin(MY_PROFILE_USER_EMAIL, MY_PROFILE_USER_PASSWORD);

  const myProfilePage = new MyProfilePage(page);
  await myProfilePage.openMyProfile();
  await myProfilePage.openAppraisalHistoryTab();

  const downloadPath = await myProfilePage.downloadAppraisalHistoryPdf();
  expect(downloadPath.toLowerCase()).toContain('.pdf');
});
