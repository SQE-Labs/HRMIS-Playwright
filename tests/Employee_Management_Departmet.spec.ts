...
test.describe("'Employee Management > Department module'", () => {
    ...
    test("should display Departments tab with correct header @reg", async ({ page }) => {
    ...
    test("should display correct number of records in Departments tab @reg", async ({ page }) => {
        let { Name_count, totalNameCount } = await EmployeeDirectory.Department_No_of_records()
        expect(Name_count).toEqual(totalNameCount)
    });

    test("Validate department search bar filters results correctly @reg", async ({ page }) => {
    ...
    test("should handle invalid search input in Departments tab @reg", async ({ page }) => {
    ...
    test("Verify update department popup opens with correct header @reg", async ({ page }) => {
    ...
    test("Verify clicking cancel button closes update department popup @reg", async ({ page }) => {
        await EmployeeDirectory.clickOnUpdateIcon()
    ...
    test("Verify clicking cross icon closes update department popup @reg", async ({ page }) => {
        await EmployeeDirectory.clickOnUpdateIcon()
    ...
    test("should update department successfully and show success message @smoke @reg", async ({ page }) => {
        await EmployeeDirectory.clickOnUpdateIcon()
    ...
    test("should display Add department popup with correct header @reg", async ({ page }) => {
        await EmployeeDirectory.clickOnUpdateIcon()
        await expect(EmployeeDirectory.Pop_Up_Header).toBeVisible()
    });

    test("should close update add department popup using cancel button @reg", async ({ page }) => {
        await EmployeeDirectory.clickOnUpdateIcon()
        await expect(EmployeeDirectory.Pop_Up_Header).toBeVisible()
        await EmployeeDirectory.clickOnCancelButton(EmployeeDirectory.DepartmentCancelButton)
        await expect(EmployeeDirectory.Pop_Up_Header).toBeHidden()
    });
    test("should close update add department popup using cross button @reg", async ({ page }) => {
        await EmployeeDirectory.clickOnUpdateIcon()
        await EmployeeDirectory.clickOnCrossButton(EmployeeDirectory.CrossButton)
        await expect(EmployeeDirectory.Pop_Up_Header).toBeHidden()
    });
    test("should show validation message when department name is empty @reg", async ({ page }) => {
        await EmployeeDirectory.clickOnUpdateIcon()
    ...
    test("should add new department and show success message @smoke @reg", async ({ page }) => {
        await EmployeeDirectory.clickOnUpdateIcon()
        let Department_name = await EmployeeDirectory.generateRandomString(6);
        await EmployeeDirectory.Name_TextArea.fill(Department_name)
        await EmployeeDirectory.PopUP_Submit_button.click()
